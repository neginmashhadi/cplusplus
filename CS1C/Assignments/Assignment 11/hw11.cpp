 ...
 * CLASS      : CS 1C
 * SECTION    : November/01/2018 Thursday
*************************************************************************/
...
int main()
{
    ...
    int (*fncPtr2)(int) = hoo; 			 	// [1.2] declare and init function pointer fcnPtr2 to point to function hoo
    double (*fncPtr3)(int ,double ) = goo;  // [1.3] declare and init function pointer fcnPtr3 to point to function goo
    // (*fcnPtr1)(int) = boo;				// [1.4] assign fcnPtr1 to point to boo (This cannot be set because fncPtr
    										//		 has a different data type and has no parameter when boo does
    fncPtr2 = boo;							// [1.5] assign fcnPtr2 to point to boo, okay - function pointer signature
    										//		 matches function boo

    // calling a function using a function pointer via explicit dereference
    (*fncPtr3)(2,5.0); 					 // [1.6] call function goo(2,5.0) through fcnPtr3 via explicit dereference

    // calling a function using a function pointer via implicit dereference
    fncPtr3(2,5);						 // [1.7] call function goo(2,5.0) through fcnPtr3 implicit dereference
...
/* virtual function2() -> calls Base::fucntion2() */
/**************************************************/
...
// The program recognizes that the function1() is a virtual function and then the
//  program uses  dPtr->__vptr to get to D2's virtual function and it will look
//  up the version of the function1() is being called. since D2 function1() has
//  been set to D2::function1() the dPtr->function1() will revolve to D2::function1()

// Q#5

// We are assigning a derived class into the base class and the problem with this
// is that the attribuates of the derived class will be sliced off so they can be
// placed into the base class
