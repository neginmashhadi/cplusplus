...
SoftwareArch::SoftwareArch(long num, string name, double percent, int years, string archName)
{
	SetValues(num, name, percent, years, archName);
}
...
void SoftwareArch::SoftwareArchPrintHeader() const
{
	cout << left << setw(18) << "Name" << setw(12) << "Department" <<
		 setw(18) << "supervisor's name" << setw(8) << "raise %"
		 << "Years of experience\n" <<
