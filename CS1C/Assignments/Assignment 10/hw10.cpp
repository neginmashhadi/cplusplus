...
vector::vector(const vector& source) // copy constructor
    ...
    : elem(source.elem), vsize(source.vsize)
{
...
int main()
{
    ...
    vector v2 = v;      // [1.1] copy v to v2: what happens here?
    					//				every element of v is being assigned into
    					//				and v which was
    ...
    v3 = v4;            // [2.1] assign v4 to v3: what happens here?
    					//			v4 information is being moved into v3 and
    					//			all the information in v4 is being deleted
    ...
    //v5 = v5;            // [2.2] self assignment: what happens here, any problems?
						// 		the problem will be because this is a rvalue reference in the assignment
						//		we delete the elem in the beginning of the function and elem is still pointing
						// 		to an empty space and then we assign the rhs elem into elem and delete rhs elem
...
/*
 * difference is that a copy constructor takes its argument by lvalue reference
 * and a move constructor by rvalue reference.
 * the function will allow the user to enter the integers and creates a temp object
 * is created and the object assign those to v6 which results in move assignment.
 * move constructor is invoked. it does result in improved performance because we will
 *  not be making extra copy of the object
 */
